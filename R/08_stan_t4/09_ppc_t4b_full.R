...
# -----------------------------------------------------------------------------
plot_rho_forest <- function(fit, fit_enso = NULL) {
  ...
  # Optionally append ENSO row (only jurel = stock 3) from the basin-scale refit.
  # See R/08_stan_t4/14b_fit_t4b_full_enso.R for the ENSO fit definition.
  if (!is.null(fit_enso)) {
    dr_enso <- fit_enso$draws("rho_enso[3]", format = "draws_df") %>%
      pivot_longer(`rho_enso[3]`, names_to = "var", values_to = "rho") %>%
      mutate(
        stock_idx = 3L,
        covar     = "ENSO",
        stock_id  = T4B_FULL_STOCKS[3],
        stock_lbl = STOCK_LABELS[T4B_FULL_STOCKS[3]]
      ) %>%
      group_by(stock_lbl, covar) %>%
      summarise(med = median(rho),
                q05 = quantile(rho, 0.05),
...
if (isTRUE(getOption("t4b.full.ppc.run_main", FALSE))) {
  ...
  L  <- ppc_full_load_all()
  # Load ENSO refit (Appendix E lag-1) if available, to overlay rho_enso[3] on
  # the rho-shifters forest plot. Falls back gracefully if ENSO fit not yet run.
  enso_fit_path <- file.path(T4B_FULL_OUT_DIR, "t4b_full_enso_lag1_fit.rds")
  fit_enso <- if (file.exists(enso_fit_path)) readRDS(enso_fit_path) else NULL

  p1 <- plot_smooth_vs_obs_full(L$fit, L$obs_df)
  p2 <- plot_residuals_full(L$fit, L$stan_data)
  p4 <- plot_omega_forest_full(L$fit)
  p5 <- plot_rho_forest(L$fit, fit_enso = fit_enso)
