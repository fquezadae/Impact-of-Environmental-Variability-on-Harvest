...
--- USE DIFFERENT LENGHT FOR FUNCTIONS??? INTEAD OF GEAR?
--- Shift in spatial distrution of jack mackerel out of EEZ during El Niño events
...
See Figure \@ref(fig:switchStrategy-ART) and Table
\@ref(tab:strategy-table-logbooks_ART) for strategy transitions. The year 2019 is used as reference as anchoveta and jack mackerel started to recover.


```{r strategy-table-logbooks_ART, echo=FALSE, results='asis'}
...
strategy_table_art %>%
  kable(
    ...
    col.names = c("Strategy", "n (Before)", "Perc (Before)", "n (After)", "Perc (After)"),
    escape = FALSE,
...
```{r strategy-table-logbooks_IND, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, results='asis'}
...
strategy_table_ind %>%
  kable("latex", booktabs = TRUE, digits = 1,
        ...
        caption = "Comparison of Strategies Before and After (IND Fleet)") %>%
  kable_styling(font_size = 10)

```


```{r switchStrategy-ART, echo=FALSE, fig.height=6, fig.width=8, message=FALSE, warning=FALSE, results='asis'}
...
```{r switchStrategy-IND, echo=FALSE, fig.height=6, fig.width=8, message=FALSE, warning=FALSE, results='asis'}
...
```
 ...
 -   Environmental covariates -- downloaded from Copernicus Marine Service using API **copernicusmarine**
        - Historical daily data (2012–2024):
          - Use Copernicus GLORYS12V1 (ocean reanalysis, ~8 km) → SST, salinity, currents  (speed and direction).
          - Use ERA5 (atmosphere reanalysis, 25 km) → winds (speed and direction).
          - Use GlobColour (Copernicus ocean colour, 4 km) → chlorophyll.
