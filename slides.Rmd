...
**Some information:**
- 2012–2024
- Chilean EEZ (32°S to 41°S)
- Coarsest resolution: ~10 km
...
The price of each species is modeled using an Inverse Almost Ideal Demand System (IADS). The log of the price $p_{iy}$ of a species $i$ in year $y$ is the following:
...
```{r sur_table, message=FALSE, warning=FALSE, include=FALSE}
library(dplyr)
library(knitr)
library(kableExtra)
library(gt)
...
- Start soon with total annual trips 
  - Hopefully a short paper can came up from that work
- Two undergrad students working on the inverse demand (i.e. price) module for their thesis... 
  - Results by July 2026
  - If they have time, they will also analyze long-run and short-run dynamics with a VEC
  - Write paper from their dissertation
