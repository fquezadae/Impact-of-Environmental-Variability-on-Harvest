...
- SPF have been used primarily for fishmeal and fish oil production [@Pena-Torres2017-gn] (~85% of jack mackerel for reduction)
...
From E.U. Copernicus Marine Service Information: 

- Global Ocean Physics Reanalysis `r Citep(bib, "GLORYS12V1", .opts = citeopt)`
  - Daily salinity, sea surface temperature, and current speed and direction
- Global Ocean Hourly Reprocessed Sea Surface Wind and Stress from Scatterometer and Model dataset `r Citep(bib, "WIND_GLO_PHY", .opts = citeopt)`
    - Hourly wind speed and direction at the surface
- Global Ocean Colour dataset `r Citep(bib, "GlobColour", .opts = citeopt)` 
    - Chlorophyll-a concentration

Some information:
- 2012–2024
- Chilean Exclusive Economic Zone (EEZ) between 32°S and 41°S
- Coarsest resolution: ~ 10km
...
- Quota prices (auction or secondary markets, if available)
  - Captures forward-looking behavior and information [@Birkenbach2024]. @reimer2022structural
  - Simplify the dynamic model to a static one.
...
- $\mathbf{X}^{'}_{vg} = [w;h_{vg};x;Z_v;Env]$ is a matrix of explanatory variables, and $\mathbf{X}_{ivg}$ represents the *i*th column of the $\mathbf{X}_{vg}$:
...
where 
- $U_{vg}=[p,w,h_{vg},\bar{q},Z_{vg}, Env]$ is a matrix of explanatory variables, 
...
- $Env$ include variables that reflect annual weather conditions
  - Accumulation of *bad weather days*? or *number of storms*?
...
Two undergrad students working on this module for their thesis... 
  ...
  - If they have time, analyze long-run and short-run dynamics with a VEC
...
- Effect on prices, at least on the short-run
...
```{r sur_table, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
library(dplyr)
library(knitr)
library(kableExtra)
...
kbl(
  ...
  format = "html",
  align  = c("l","r","r","r"),
  col.names = c("", "<i>Sardine</i>", "<i>Anchoveta</i>", "<i>Jack mackerel</i>"),
  ...
  footnote(
    general = "Entries are coefficients; significance: * p<0.10, ** p<0.05, *** p<0.01. Quadratic terms shown with “sq”; interactions use ×.",
    threeparttable = TRUE
  )
```
