...
Output prices $p_{sy}$ correspond to species-specific ex-vessel prices paid by processing plants to fishers, obtained from IFOP's manufacturing survey and deflated to constant pesos using the consumer price index. Consistent with @dresdner2013, prices are measured as annual averages over peak fishing months to reflect the economically relevant conditions faced by vessels when planning annual effort.
...
Quota shares do not enter Eq.~\eqref{eq:poisson_trips} directly. Instead, shares are used to translate annual TACs into vessel-level allocated harvests, which then enter the trip equation. Let $\omega_{vs}$ denote vessel $v$'s historical share of landings of species $s$ within the purse-seine fleet, computed over the full sample period. Given the TAC $\bar{Q}_{sy}$ for species $s$ in year $y$, vessel-level allocated harvest is defined as:
...
stargazer(
  ...
  covariate.labels = c(
    ...
    "Allocated harvest ($H^{alloc}_{vy}$)",
    "Price jack mackerel",
    "Price sardine",
    "Price anchoveta",
    "Bad weather days",
  ...
  notes = c(
    "$^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01.",
    "Vessel-clustered robust SEs. Prices in constant 2018 pesos/ton.",
...
The environmental and regulatory variables show distinct patterns across fleets. For the industrial fleet, adverse weather days have a marginally significant negative effect ($p = 0.05$), while biological closure days are strongly negative and represent the largest effect in the model. This is consistent with the industrial fleet's dependence on sardine-anchoveta access during open seasons. For the artisanal fleet, adverse weather days are not significant at the 10 m/s threshold, suggesting that smaller vessels may be constrained by weather at lower wind speeds. The positive coefficient on closure days in the artisanal model reflects the current limitation that this variable varies only across regulatory zones (151 days for regions V--VIII versus 182 days for regions IX--XIV) and therefore captures locational differences in fishing activity rather than the causal effect of closures. Once year-specific closure dates from annual SUBPESCA resolutions are incorporated, this coefficient is expected to become negative.
