...
**Just use real SPF trips (consider 95% of total revenue...)**

- Variables:
  - Input prices
  - Output prices (by species) -- Vessels' average prices foe each species weighted by the proportion of each product form
  - Annual harvest allocated to each vessel (vessel/species) -- MAYBE SE SHARES????
  - Annual TAC level (by species)
...
[@Kasperski2015-jm]: 
\begin{equation}
Pr\left[T^{*}_{vy} = t_v\right] = \frac{exp^{-exp(U^{'}_{v}\beta)}exp(U^{'}_{v}\beta)^{t_v}}{t_v !}
\label{eq5}
\end{equation} 

where $U_{v}=[p,w,h_{v},\bar{q},Z_{v}]$ is a $(3n+M+k+1)×V_g$ matrix of explanatory variables, $\beta$ is a $(3n+M+k+1)\times1$ matrix of coefficients to be estimated, $t_v$ is the number of trips taken by vessel $v$ using gear type $g$ in year $y$, and $\bar{q}$ is the annual quota level. Additionally, we can add the accumulation of "bad weather days" as an explanatory variable to incorporate weather conditions into this decision, thus $U_{vg}=[p,w,h_{v},\bar{q},Z_{v}, Env]$

WHERE ISN THE SHARE OF THE QUOTA? QUOTA FOR EACH SPECIE???
...
``The estimates for each vessel-type combination are used to determine the optimal
number of trips each vessel will take over the course of a year in the numerical optimization
as a result of **changing prices, quotas, and individual annual harvests**. Combining
the number of trips each vessel takes, each vessel’s share of the total annual quota, and the cost functions associated with their trip
level harvest, the model can now calculate the total cost of harvesting any level of output.
The following section estimates an inverse demand model for all species jointly to provide
estimates of the price impacts of increased harvesting.''

Hay ENDOGENEIDAD??? NUMERO DE VIAJES CON PRECIO???? Y CANTIDAD???


```{r poisson, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
pois_trips <- glm(
  trips ~ log(harvest_total) + log(p_diesel) + log(w_crew) + factor(gear) + Z,
