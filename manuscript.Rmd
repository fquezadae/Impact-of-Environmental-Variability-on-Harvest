...
Allowable Catch (TAC; *Cuota Global*), which is divided between the
...
(*Licencias Transables de Pesca, LTP*). Class A licenses were allocated
...
To estimate stock dynamics, we use annual data on species-specific
biomass (stock abundance) and vessel landings within Chile’s Exclusive
Economic Zone (EEZ). Following @Kasperski2015-jm, the baseline model for
the interannual growth of each species $i$ is represented by a discrete
logistic function that accounts for intra- and inter-species
interactions:
...
\label{eq1}
\end{equation} where $x_{iy}$ is the fish stock by species
$i=1,\ldots,n$ in year $y$, $n$ is the total number of species, $h_{iy}$
is the annual harvest of species $i$ on year $y$, $r_i$ is the intrinsic
growth rate of the resource $i$, $\eta_i$ is a density-dependent factor
related to the carrying capacity, and $\alpha_{ij}$ are the interaction
parameters between species. The system of $n$ growth equations is
estimated simultaneously using seemingly unrelated regression (SUR).
Following @Richter2018, we augment \eqref{eq1} by including
environmental covariates $Env_{iy}$ that affect the fish stock and an
error term $\varepsilon_{iy}$ that captures random recruitment:
\begin{equation}
...
Environmental conditions were summarized annually using sea surface
temperature (SST) and chlorophyll-a concentration (CHL), both averaged
over the South-Central Chile region within the EEZ. These variables were
selected due to their recognized influence on small pelagic productivity
and spatial distribution [@Axbard2016]. To capture potential nonlinear
responses, both linear and quadratic terms for SST and CHL were
included. Early specifications also tested linear and quadratic wind
effects, but excluding them did not significantly reduce explanatory
power ($\text{F}_{6,24} = 0.36$, $p\text{-value} = 0.89$). SST serves as
a proxy for large-scale oceanographic regimes (warm vs. cold phases)
that shape recruitment success and species dominance along the Humboldt
Current System. Anchovy tends to dominate during cold, nutrient-rich
phases, while sardine prevail under warmer conditions [@cahuin2009;
@Yáñez2014]. CHL captures interannual variations in primary
productivity, which are critical for sustaining the planktonic base of
pelagic food webs. As emphasized by @sumaila2011, “Changes in primary
productivity and planktonic community structure affect the amount of
energy transferred to higher trophic levels and, eventually, the
productivity of trophic groups that contribute to fisheries catches”
[see also @jennings2008; @cheung2008]. Thus, chlorophyll acts as an
indicator of ecosystem energy available to forage fish.

The SUR framework is particularly suited to this multi-species context,
as it allows for contemporaneous correlation among residuals that arise
from shared environmental shocks and trophic linkages. Cross-species
interaction terms are specified between predator (jack mackerel) and
prey (anchoveta, sardine) pairs, enabling the model to capture both
biological coupling and indirect effects of environmental variability
transmitted across species.

-   To add here:
    -   Some species might be more resistant to changes in SST and CHL
    -   Jack Mackerel biomass why doifferent than Anchoveta and Sardine
        biomass..
    -   Sardine and Anchoveta they move together
    -   BE CLEAR THAT EACH SPECIES HAVE ITS OWN EQUATION
-   PROBAR CON SOLO INTERACCION CON MACKEREL!!! (Pray and predator
    model)
...
stargazer(
  ...
  covariate.labels = c(
    ...
    "Clorofila-a",
    "Clorofila-a$^2$",
    "Interacción sardina--anchoveta",
    "Interacción sardina--jurel",
    "Interacción anchoveta--jurel",
    "Constante"
  ),
  ...
  notes = "*p$<0.1$; **p$<0.05$; ***p$<0.01$. Errores estandar robustos (MLR, lavaan).",
  notes.append = FALSE
)
