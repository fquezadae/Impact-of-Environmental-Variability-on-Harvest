...
```{r ind_harvest_substitution_SERNAPESCA, message=FALSE, warning=FALSE, include=FALSE}
library(readxl)
library(dplyr)
library(tidyr)
library(purrr)
...
```{r strategy-table, echo=FALSE, results='asis'}
library(knitr)
...
```{r ind_harvest_substitution_IFOP, message=FALSE, warning=FALSE, include=FALSE}
...
CALCULAR HARVEST IFOP CON BITACORAS!!!
```
...
harvest_long <- harvest %>% 
  dplyr::select(c("specie", "year", "total_harvest_IFOP_centrosur", "total_harvest_all_SERNAPESCA_Centro_Sur")) %>%
