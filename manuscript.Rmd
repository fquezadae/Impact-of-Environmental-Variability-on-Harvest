...
- Composed by anchoveta (*Engraulis ringens*), sardine (either *Sardinops sagax* or *Strangomera bentincki*)
- Other important species is jack mackerel (*Trachurus murphyi*). Jack mackerel is catched mostly by small-scale boats (race for fish). 
- Unfortunately, due to lack information about stock biomass, we could not include this species in our research
...
- Some facts:
  ...
  - Anchoveta and sardina is a mix species, with a mixed regulation. Separate quota, but if a vessel can´t found a species, can use the quota for that species to capture the other one.
...
```{r ind_harvest_substitution, message=FALSE, warning=FALSE, include=FALSE}
...
get_strategy <- function(sardine, jackmackerel, anchoveta) {
  species <- c()
  if (sardine > 0.25) species <- c(species, "Sardine")
  if (jackmackerel > 0.25) species <- c(species, "JackMackerel")
  if (anchoveta > 0.25) species <- c(species, "Anchoveta")
...
vessel_strategies_yearly <- vessel_species_year %>%
  mutate(strategy = pmap_chr(
    list(Sardine, JackMackerel, Anchoveta),
    get_strategy
  ))

strategy_percent_ART <- vessel_strategies_yearly %>%
  group_by(strategy) %>%
...
```{r monthly_graph_harvest, echo=FALSE, fig.align='center', fig.cap="Average monthly landings by species (2012-2024; South-Central Chile)", fig.pos='ht!', message=FALSE, warning=FALSE}
...
#### HAY QUE SOLO INCLUIR CENTRO SUR EN COSECHA!!!! ####
#### VER TAMBIEN LA OTRA BASE DE DATOS DE BIOMASA!!!! ####
#### REVISAR UNIDADES!!!! ####
