...
# The manuscript no longer reads from the external data root: every input the
# knit needs is versioned in the repository (see the preflight below). The two
# daily environmental grids were loaded but never used downstream, so they have
# been dropped. FONDECYT_DATA is kept only as an optional, never-required path.
dirdata <- Sys.getenv("FONDECYT_DATA", unset = "")
if (nzchar(dirdata) && !grepl("/$", dirdata)) dirdata <- paste0(dirdata, "/")
...
# desde la raíz del proyecto y no desde paper/, evitando depender del
# junction paper/data -> ../data.
...
The Chilean SPF is managed through an annual Total Allowable Catch (TAC; \emph{Cuota Global}), further subdivided across regions and seasons, with unused quotas reassignable within the fishing year and small fractions reserved for research and contingency. Anchoveta and sardine operate under a mixed-species quota framework in which each species has an independent quota but substitution between them is permitted. Because the two species are jointly harvested using the same gear and fishing grounds and are biologically indistinguishable at the point of capture [@dresdner2013], the species mix of landings depends on the quota allocation rather than on biology or fishing technology.
...
```{r results-identification, child='paper/sections/results_identification.Rmd'}
```
...
Under the moderate SSP2-4.5 mid-century scenario---warming of $+0.58\,^{\circ}$C and small chlorophyll-a changes (cross-model medians)---anchoveta's intrinsic productivity declines by $51\%$ and sardine's by $79\%$, with posterior probability of negative impact equal to one in both cases.

Under the high-emissions SSP5-8.5 end-of-century scenario---warming of $+2.39\,^{\circ}$C and small chlorophyll-a changes on average ($\Delta\log\text{CHL}$ median $-0.013$, but with a $5$th-percentile of $-0.30$ across models)---the cross-model medians deepen to $-90\%$ and $-99.9\%$ respectively, and the entire $90\%$ credible interval of sardine's response lies below zero in every model of the ensemble. The anchoveta cross-model probability of decline is $0.99$ rather than $1$ because anchoveta's CHL semi-elasticity is negative ($\rho^{CHL}_{\text{anch}} \approx -3.65$): in the small fraction of (model, draw) pairs where $\Delta\log\text{CHL}$ is sufficiently negative, the CHL contribution becomes positive and offsets the SST-driven collapse.
...
gcs_kable <- kable(gcs_tbl, format = "latex", booktabs = TRUE, escape = FALSE,
      align = "llccccccr",
      caption = "Comparative statics on intrinsic stock productivity ($\\Delta r_s^{\\star}/r_s^{0}$) under the CMIP6 six-model ensemble. \\label{tab:growth_compstat}") %>%
...
The comparative-statics exercise is a long-run object rather than a year-by-year forecast. It describes the steady-state intrinsic productivity of each stock under a counterfactual climate, *holding fixed* the law of motion and the fishery's regulatory regime. The shifters $(\rho_s^{SST}, \rho_s^{CHL})$ enter the law of motion as coefficients on exogenous climate forcings. Because they are structural parameters, not reduced-form correlations, they remain valid at climate regimes outside the estimation window---in particular, at an SST anomaly of $+2.3^{\circ}$C that lies roughly nine historical standard deviations beyond the 2000--2024 mean.
...
kable(tcs_emb_tbl, format = "latex", booktabs = TRUE, escape = FALSE,
  ...
  kable_styling(latex_options = c("hold_position"), font_size = 9) %>%
  footnote(general = "The artisanal negative binomial estimation is restricted to vessel-type categories with at least 50 vessel-year observations (BM, LM, UNK); excluded categories collectively account for less than 0.1 percent of artisanal effort and quota.",
