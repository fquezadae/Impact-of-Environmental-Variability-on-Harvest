...
- Year FE: $\beta_w^{ART}$ halves vs no-FE spec ($-0.0018 \to -0.0008$) &mdash; estallido + COVID were contaminating $\beta_w$.

## Two channels to projections

- **Indirect**: structural shifters $\rho^{SST}_i, \rho^{CHL}_i \to B_{i,t} \to H^{alloc}_{vy}$.
- **Direct**: vessel-specific $\Delta$bad-weather days from CMIP6 winds, evaluated at $\beta_w^g$.
]
...
<span class="small-note">Negative binomial **with year FE** (absorbs estallido + COVID), vessel-clustered robust SEs. Sign and significance shown. <sup>***</sup> $p < 0.01$. <sup style="color:#c4830a;">&dagger;</sup> zone-FE collinear (not causally interpretable). $\beta_w^{ART}$ halves once year FE is in &mdash; aggregate-shock contamination, not climate.</span>
...
<table style="width:100%; font-size:0.78em; border-collapse:collapse; margin:8px 0;">
<thead>
...
<tr style="border-bottom:2px solid #113b63;">
<td style="padding:6px 10px;">end-century, severe (SSP5-8.5)</td>
<td style="text-align:right; padding:6px 10px;">&minus;1.1%</td>
...
$\%\Delta T$: change in annual trips vs baseline. **Marginal** averages over all posterior draws (incl. portfolio-collapse paths $\to$ floor effect). **Conditional** restricts to draws with $f^H_v > 0.5$. Cross-median across a 6-model CMIP6 ensemble. NB with year FE.
</span>
...
# Direct vs indirect channel &mdash; how does climate hit effort?

.pull-left[
## Two channels, decomposed
...
.pull-right[
## Channel split, end-century SSP5-8.5

| Fleet | Indirect | Direct | Total |
|---|---:|---:|---:|
| **Artisanal** | $\approx -9.0$ pp | $\approx -1.2$ pp | $-10.2\%$ |
| **Industrial** | $\approx -1.0$ pp | $\approx -0.1$ pp | $-1.1\%$ |

<span class="small-note">Approximate channel decomposition; built by zeroing one channel at a time and re-running the comparative statics. Direct-channel asymmetry follows from $\beta_w^{ART} = -0.0008^{***}$, $\beta_w^{IND} \approx 0$ (year-FE NB).</span>
]

<div style="height: 6px;"></div>

.tldr[Indirect (biomass) carries **most** of the asymmetry; direct (weather) reinforces it where vessels are exposed (ART). Both channels point the same direction &mdash; floor effect in sardine dominates the marginal $\%\Delta T$.]
...
# Appendix D &middot; Why is jack mackerel n.i.? (triple evidence)

.tight[
**(i) Spatial-domain robustness.** Re-fits over 3 alternative coastal domains (D1/D2/D3): $\sigma_{post}/\sigma_{prior}$ for $\rho^{SST}_{jurel}$ stays in [0.998, 1.014] &mdash; data adds no information regardless of domain. Anchoveta and sardine remain stable (CV $< 0.03$) across the same exercise.

**(ii) Dual-source biomass.** Augmenting the CS state-space with the IFOP Northern Chile acoustic series (2010&ndash;24, $r_{CS,N}^{log} = 0.88$) does not break non-identification: $\sigma_{post}/\sigma_{prior} \in \{0.94, 0.99\}$ for $(\rho^{SST}, \rho^{CHL})$. The bottleneck is **structural**, not sample size.

**(iii) OROP-PS coherence.** SPRFMO transzonal index correlates with CS at $r = 0.11$ over $N = 17$ overlapping years. CS is decoupled from the basin-scale signal &mdash; range-wide forcing cannot pin it down either.
