...
- **Indirect**: structural shifters $\rho^{SST}_i, \rho^{CHL}_i \to B_{i,t} \to H^{alloc}_{vy}$. *Jurel: testing $\rho_{ENSO}$ over Ni&ntilde;o 3.4 instead* (App. D-bis).
...
.pull-right[
- IND's protection rests on jack mackerel (long-run *local* shifter **n.i.**; ENSO test in progress, App. D-bis).
]
...
<span class="small-note">*Jurel: extension to $\rho_{ENSO}$ over Ni&ntilde;o 3.4 in progress (App. D-bis).*</span>
...
<span class="small-note">*Stock-specific extension in progress: jurel forced by $\rho_{ENSO}$ over Ni&ntilde;o 3.4 instead of local $(SST_{D1}, \log CHL_{D1})$ &mdash; see App. D-bis.*</span>
...
<span class="small-note">Posterior densities of $\rho^{SST}$ and $\rho^{CHL}$, full multi-species specification. Anchoveta and common sardine: priors dominated by data. Jack mackerel: posteriors track priors $\to$ **n.i.** at the *local* coastal scale &mdash; **basin-scale ENSO shifter being tested next** (App. D-bis).</span>
...
<span class="small-note">Cross-median across CMIP6 ensemble of 6 GCMs (CESM2, CNRM-ESM2-1, GFDL-ESM4, IPSL-CM6A-LR, MPI-ESM1-2-HR, UKESM1-0-LL); 90% CI is the within-posterior credible interval averaged across models. *Jurel: local $(SST_{D1}, \log CHL_{D1})$ shifter n.i. &mdash; basin-scale ENSO refit in progress (App. D-bis).*</span>
...
$\Rightarrow$ The *local* null is robust. Natural reading: **relevant climatic drivers operate at scales beyond CS coastal forcing** &mdash; consistent with a transboundary species. Tested next on the obvious candidate.
...
.tldr[Status: data extraction in progress; Stan refit by mid-May.]

---

# Appendix D-ter &mdash; ENSO: asymmetric bet on the result

.pull-left[
## Scenario A &mdash; identifies

$\sigma_{post}/\sigma_{prior} \le 0.7$:

- Null becomes **basin-scale identification**.
- Asymmetry **11:1 recomputed** with structural base for *all 3* stocks.
- Direct test of Pe&ntilde;a-Torres 2017 / Arcos 2001 mechanism.
]

.pull-right[
## Scenario B &mdash; not identified

$\sigma_{post}/\sigma_{prior} \approx 1$:

- Null moves from 3 to **4 converging lines of evidence**.
- Climate effects on jurel operate on *margins* (location, behavior), not local productivity.
]

<div style="height: 12px;"></div>

.tldr[<b style="color:#c4830a;">&rarr;</b> Either scenario improves the paper.]
