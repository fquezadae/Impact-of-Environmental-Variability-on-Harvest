...
kable(loo_tbl, format = "latex", booktabs = TRUE, escape = FALSE,
  ...
  footnote(general = paste(
      "$\\Delta\\widehat{\\text{ELPD}}$ is the pairwise difference relative to T4b-full (the leading model);",
      "SE$_{\\Delta}$ is the standard error of that difference.",
      "After moment matching, 65/68 observations satisfy $\\hat{k}\\leq 0.7$ for T4b-full,",
      "64/68 for T4b-ind, and 67/68 for T4b-omega. The remaining high-$\\hat{k}$ points are",
...
kable(lfo_tbl, format = "latex", booktabs = TRUE, escape = FALSE,
  ...
  footnote(general = paste(
      ...
      "Each refit uses 4 chains $\\times$ 1500 warmup + 1500 sampling iterations.",
      "All 15 refits satisfied $\\hat{R} \\leq 1.01$ with at most one divergent transition",
      "(T4b-omega at $t_{\\text{cut}}=2011$), which does not affect the reported ELPD."),
      general_title = "Note:", escape = FALSE, threeparttable = TRUE)
```
