...
Table \ref{tab:growth_compstat} reports $\Delta r_s^{\star}/r_s^{0}$, the climate-driven shift in long-run productivity defined in Section \emph{\nameref{projection-approach}}, for each stock under each scenario-window pair. We separate two sources of uncertainty. \emph{Climate uncertainty}---variation across CMIP6 models in projected SST and CHL deltas (the only climate inputs to the stock-dynamics equation)---is summarised by the cross-model median (column $\Delta r_s^{\star}/r_s^{0}$) and inter-model IQR (column \emph{Cross-model IQR}). \emph{Structural uncertainty}---variation across posterior draws of the climate shifters $(\rho_s^{SST}, \rho_s^{CHL})$---is summarised by the median across models of the within-model 90\% posterior credible interval (column \emph{Within posterior 90\% CI}). A separate variance decomposition appears in Online Appendix F. Jack mackerel is reported as non-identified (``n.i.'') consistent with the identification result in Section \emph{\nameref{identification}}.
