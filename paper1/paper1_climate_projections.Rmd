...
At the vessel level we aggregate the species-specific biomass shifts into a single \emph{composition-weighted biomass factor} relative to the historical baseline,
\begin{equation}
f^H_v \;=\; \sum_s \omega_{vs}\,\frac{B_s^{\star}}{B_s^{0}}, \label{eq:fHv}
\end{equation}
where $\omega_{vs}$ is vessel $v$'s realised catch share in species $s$ over the historical sample and $B_s^{\star} = K_s(1 - F_s^{\text{hist}}/r_s^{\star})$ is the Schaefer steady-state biomass under the projected climate, holding fishing pressure at the 2000--2024 average. Jack mackerel is treated as non-identified ($B_{\text{jur}}^{\star}/B_{\text{jur}}^{0}=1$) consistent with Section \emph{\nameref{identification}}. We summarise the resulting fleet-level percentage change in trips, $\%\Delta T_f$, with two posterior medians that isolate complementary economic content. The \emph{marginal} median integrates over all posterior draws within each model and captures the full response, including portfolio collapse. The \emph{conditional} median restricts the integration to draws with $f^H_v \geq 0.5$, isolating the pure climate-elasticity component from the steady-state collapse component; we also report the posterior probability of portfolio loss, $\Pr(f^H_v<0.5)$, as a stand-alone measure of collapse risk. Both medians are then aggregated across CMIP6 models via cross-model median and IQR, in parallel with the productivity statistics above.
...
kable(tcs_tbl, format = "latex", booktabs = TRUE, escape = FALSE,
  ...
  footnote(general = paste(
    "SSP2-4.5 panels use 5 instead of 6 CMIP6 models because one model does not publish chlorophyll-a projections for that scenario.",
    "Raw values, by-model disaggregation, and by-stock extinction probabilities are in tables/trip_comparative_statics_raw.csv, ..._by_model.csv, and ..._extinct.csv."),
    general_title = "Note:", escape = TRUE, threeparttable = TRUE)
