...
editor_options:
  markdown: 
    wrap: 72
abstract: "Climate-driven shifts in stock productivity reach fishing fleets unevenly when species portfolios differ across fleet segments and quota allocation rules limit cross-sector transferability. We quantify this distributional channel for Chile's multi-species small pelagic fishery (anchoveta, sardine, jack mackerel), managed under the *Límite Máximo de Captura por Armador* (LMCA). A bioeconomic framework couples a Bayesian state-space model of stock dynamics, calibrated on official assessments for 2000--2024, with a vessel-level trip equation including year fixed effects; climate propagates through an indirect biomass channel and a direct weather channel. Evaluated against a six-model CMIP6 ensemble across two emissions scenarios and two time windows, the framework projects that fleet-level effort falls by 8--10 percent for the artisanal fleet but only 0.7--0.9 percent for the industrial fleet, an asymmetry of roughly eleven to one driven by differential portfolio exposure and weather sensitivity. The findings imply that climate-adaptation policy in Chile should target cross-sector quota transferability rules to attenuate disproportionate fleet-level impacts."
---
