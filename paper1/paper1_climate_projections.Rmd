...
editor_options:
  markdown: 
    wrap: 72
abstract: "We project the differential climate-change impacts on fishing effort across the industrial and artisanal segments of Chile's multi-species small pelagic fishery (anchoveta, sardine, jack mackerel), managed under a quota regime with limited cross-sector transferability. The bioeconomic framework couples a Bayesian state-space model of stock dynamics, calibrated on official assessments for 2000--2024, with a vessel-level trip equation that includes year fixed effects. Climate change propagates through two channels: an indirect biomass channel and a direct weather channel. Using a six-model CMIP6 ensemble across two emissions scenarios and two time windows, we project that fleet-level effort falls by 8--10 percent for the artisanal fleet but only 0.7--0.9 percent for the industrial fleet, an asymmetry of roughly eleven to one driven by differences in portfolio composition and weather sensitivity. Climate-adaptation policy should therefore target the design of cross-sector quota transferability rules to attenuate disproportionate fleet-level impacts."
---
...
Our empirical strategy is structural in the Cowles sense. We identify---rather than merely estimate---a vector of climate shifters that modulate the intrinsic growth rate of each stock within a Bayesian state-space model of multi-species stock dynamics, calibrated on official IFOP and SPRFMO assessments for 2000--2024. The identified parameters are interpretable as semi-elasticities of stock productivity with respect to sea-surface temperature and log-chlorophyll-a anomalies. Because they are invariant to the joint historical distribution of biomass, harvest, and environmental conditions, they are transportable to climate regimes outside the estimation envelope. We then couple this structural response with a vessel-level negative binomial trip equation, estimated separately for the industrial and artisanal fleets with year fixed effects that absorb aggregate non-climate shocks (notably the 2019--2020 social and pandemic disruptions). Climate propagates to vessel effort through two channels: an indirect channel via the structural biomass shifters and a direct channel via vessel-specific exposure to severe winds. The structural model is evaluated at CMIP6 projections from a six-model ensemble (IPSL-CM6A-LR, GFDL-ESM4, CESM2, CNRM-ESM2-1, UKESM1-0-LL, MPI-ESM1-2-HR) under SSP2-4.5 and SSP5-8.5, at mid-century (2041--2060) and end-of-century (2081--2100), with cross-model and within-posterior uncertainty separated explicitly.

We obtain three sets of results. First, the climate semi-elasticities are sharply identified for anchoveta and sardine---the two coastal upwelling stocks---but structurally non-identified for jack mackerel, a finding we document through a triple-evidence package (spatial-domain robustness, dual-source biomass, and OROP-PS coherence) and treat consistently throughout. Second, the long-run change in stock-specific intrinsic productivity is large and negative for the two identified stocks, with floor effects for sardine under SSP5-8.5 end-of-century. Third, the projected fleet-level effort response is asymmetric: the artisanal fleet contracts by 8.1--10.2\% and the industrial fleet by only 0.7--0.9\%, a ratio of roughly eleven to one. This asymmetry arises from the interaction between each fleet's portfolio exposure to the identified shifters and the limited cross-sector transferability built into the LMCA. The mechanism is consistent with the broader portfolio literature on income diversification and risk in fisheries [@Kasperski2013-jz; @Cline2017-dp] and with evidence that institutional access constraints can limit the realized benefits of diversification [@Oken2021-of]. Because adaptation responses by fishers are themselves heterogeneous and context-dependent [@Zhang2011-wv; @Jardine2020-um], the policy reading is clear: the design of cross-sector transferability rules within the LMCA is a first-order margin for attenuating disproportionate fleet-level impacts. A within-versus-between variance decomposition (Appendix \ref{appendix-trips-ensemble}) shows that 97--100\% of the projected dispersion in fleet-level effort change is within-posterior rather than across CMIP6 models, so the asymmetry conclusion is robust to the climate-model uncertainty that dominates analogous projections at the stock-productivity level.
...
The comparative-statics exercise is a long-run object rather than a year-by-year forecast. It describes the steady-state intrinsic productivity of each stock under a counterfactual climate, *holding fixed* the law of motion and the fishery's regulatory regime. This is precisely what the structural identification of the shifters permits: because $(\rho_i^{SST}, \rho_i^{CHL})$ are invariant to the joint distribution of $(SST, B, C)$, they can be evaluated at climate regimes outside the estimation window---in particular, at an SST anomaly of $+2.3^{\circ}$C that lies roughly nine historical standard deviations beyond the 2000--2024 mean. Reduced-form correlational specifications do not license this extrapolation.
