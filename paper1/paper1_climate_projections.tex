...
\begin{tablenotes}
\item \textit{Note:} 
\item SSP2-4.5 panels use $n_m=5$ because one of the six CMIP6 models in the ensemble does not publish chlorophyll-a projections for SSP2-4.5; this matches the sub-ensemble used in Table \ref{tab:growth_compstat}.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{table}[!h]
\centering
\caption{\label{tab:growthcompstat}Comparative statics on intrinsic stock productivity under the CMIP6 six-model ensemble. SSP2-4.5 panels use $n_m=5$ because one of the six models does not publish chlorophyll-a projections for that scenario; SSP5-8.5 panels use all six. \label{tab:growth_compstat}}
\centering
...
\toprule
\multicolumn{5}{c}{ } & \multicolumn{4}{c}{Summaries of $\Delta r_s^{\star}/r_s^{0}$} \\
