...
\begin{tablenotes}
\item \textit{Note:} 
\item SSP2-4.5 panels use 5 instead of 6 CMIP6 models because one model does not publish chlorophyll-a projections for that scenario. Raw values, by-model disaggregation, and by-stock extinction probabilities are in tables/trip\_comparative\_statics\_raw.csv, ...\_by\_model.csv, and ...\_extinct.csv.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}
\item \textit{Note:} 
\item Column definitions and cross-model aggregation follow the preceding fleet-level table. Categories with fewer than 50 vessel-year observations (BR, BRV, L), collectively less than 0.1 percent of artisanal effort and quota, are excluded. BM is the reference category in the negative binomial interaction; LM and UNK enter as deviations. Raw values and other scenario-window combinations are archived in trip\_comparative\_statics\_by\_tipo\_emb\_raw.csv.
\end{tablenotes}
\end{threeparttable}
\end{table}
