...
The fact that the jack mackerel shifter is not identified at the Centro-Sur scale is itself a substantive result rather than a nuisance, but its policy reading depends on a careful distinction between \emph{non-identification} and \emph{no climate effect}. The two are conflated only by reduced-form specifications that force a coefficient onto every regressor regardless of identification. Existing empirical evidence on jack mackerel documents that climate does affect the species at scales broader than local coastal \href{mailto:anomalies---@Pena-Torres2017-gn}{\nolinkurl{anomalies-\/-\/-@Pena-Torres2017-gn}} report ENSO-driven shifts in jack mackerel location choices in the Chilean fishery, and Arcos et al. (\citeproc{ref-Arcos2001-jq}{2001}) describe SPRFMO-scale stock dynamics consistent with transboundary forcing---so the absence of an identifiable Centro-Sur shifter is most naturally read as evidence that the relevant climatic drivers operate at spatial scales beyond those captured by local SST and chlorophyll anomalies, not as evidence of climate insensitivity. The Bayesian state-space specification correctly reports this null when a stock's local productivity is decoupled from coastal forcing, as biological theory would predict for a highly migratory, transboundary species managed at the SPRFMO scale. Reduced-form specifications that force a coefficient onto this species would instead deliver a precisely estimated but structurally meaningless number, which would then be propagated into climate projections as if it were information. The state-space identification exposes the absence of \emph{local} information directly; this is the econometric value-added of Cowles-style structural inference in this setting, as opposed to correlational benchmarks. The corollary for policy is that climate adaptation for jack mackerel cannot be read off the Chilean Centro-Sur time series alone and must be assessed jointly with the SPRFMO stock assessment in future work---a conclusion that takes the broader-scale climate sensitivity documented by Peña-Torres et al. (\citeproc{ref-Pena-Torres2017-gn}{2017}) and Arcos et al. (\citeproc{ref-Arcos2001-jq}{2001}) as input, rather than as something this paper rules out.
