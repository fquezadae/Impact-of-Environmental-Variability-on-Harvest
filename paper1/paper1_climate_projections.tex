...
The Chilean SPF is managed through an annual Total Allowable Catch (TAC; \emph{Cuota Global}), further subdivided across regions and seasons, with unused quotas reassignable within the fishing year and small fractions reserved for research and contingency. Anchoveta and sardine operate under a mixed-species quota framework in which each species has an independent quota but substitution between them is permitted. Because the two species are jointly harvested using the same gear and fishing grounds and are biologically indistinguishable at the point of capture (\citeproc{ref-dresdner2013}{Dresdner et al. 2013}), the species mix of landings depends on the quota allocation rather than on biology or fishing technology.
...
\begin{tablenotes}
\item \textit{Note:} 
\item Shifters are semi-elasticities of intrinsic growth $r_s$ with respect to local SST (°C), local log-CHL (log-points), and the basin-scale Niño 3.4 index. The ENSO row reports the lag-1 refit of the basin-scale ENSO test of Online Appendix E for jack mackerel only; anchoveta and sardine inherit their main-specification posteriors. All $\hat{R} \leq 1.01$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
(\(1 - e^{-1.04 \cdot 0.26}\)) and that of sardine by approximately 51\%
(\(1 - e^{-2.76 \cdot 0.26}\)) relative to the climatological mean. Under a
...
Second, the two coastal stocks respond to chlorophyll-a with opposite signs: \(\rho^{CHL}_{\text{anch}} = -3.65\) (CI \([-5.01, -2.25]\)) versus
\(\rho^{CHL}_{\text{sard}} = +2.18\) (CI \([+0.96, +3.37]\)). At one historical
...
\begin{table}[!h]
\centering
\caption{\label{tab:growthcompstat}Comparative statics on intrinsic stock productivity ($\Delta r_s^{\star}/r_s^{0}$) under the CMIP6 six-model ensemble. \label{tab:growth_compstat}}
\centering
...
The comparative-statics exercise is a long-run object rather than a year-by-year forecast. It describes the steady-state intrinsic productivity of each stock under a counterfactual climate, \emph{holding fixed} the law of motion and the fishery's regulatory regime. The shifters \((\rho_s^{SST}, \rho_s^{CHL})\) enter the law of motion as coefficients on exogenous climate forcings. Because they are structural parameters, not reduced-form correlations, they remain valid at climate regimes outside the estimation window---in particular, at an SST anomaly of \(+2.3^{\circ}\)C that lies roughly nine historical standard deviations beyond the 2000--2024 mean.
...
\begin{tablenotes}
\item \textit{Note:} 
\item The artisanal negative binomial estimation is restricted to vessel-type categories with at least 50 vessel-year observations (BM, LM, UNK); excluded categories collectively account for less than 0.1 percent of artisanal effort and quota.
\end{tablenotes}
\end{threeparttable}
\end{table}
