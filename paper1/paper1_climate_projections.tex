...
full posterior diagnostics are reported in Appendix \ref{appendix-posterior}.
...
Table \ref{tab:trip_compstat} translates the comparative-statics object on intrinsic stock productivity reported in Table \ref{tab:growth_compstat} into a comparative-statics object on fleet-level annual trips, by propagating each posterior draw through the Schaefer steady-state biomass equation under historical average fishing pressure and through the negative binomial trip equation of Section\textasciitilde{}\ref{sec:tripresults}. Vessel-level responses are aggregated to the fleet level as medians over posterior draws and vessels within fleet; the aggregation weights are each vessel's realized catch composition over the 2012--2024 estimation window.
...
state-space specification of Section\textasciitilde{}\ref{sec:stock-dynamics}. The
...
Table\textasciitilde{}\ref{tab:stress-mape} reports the median absolute percent error
...
a finding of strong climate forcing. Figure\textasciitilde{}\ref{fig:stress-traj}
...
Eq.\textasciitilde{}\eqref{eq:stress-shifter} returns values close to one for both
...
Eq.\textasciitilde{}\eqref{eq:stress-law}, not features of the design matrix.
...
Section\textasciitilde{}\ref{identification}, where the marginal posterior of
...
likelihood of Section\textasciitilde{}\ref{sec:stock-dynamics} are reported in
Table\textasciitilde{}\ref{tab:stress-priors}. For each coastal stock, the prior centre
is taken from the joint MLE of Section\textasciitilde{}\ref{sec:appendix-stress-results},
...
scale, as discussed in Section\textasciitilde{}\ref{identification}.
...
Appendix\textasciitilde{}\ref{appendix-convergence}.
...
Section\textasciitilde{}\ref{identification}. The fit was run with four chains of
...
in Table\textasciitilde{}\ref{tab:convergence}.
...
Section\textasciitilde{}\ref{identification} are not driven by under-mixed chains.
...
Table\textasciitilde{}\ref{tab:convergence}, the diagnostic battery supports the
