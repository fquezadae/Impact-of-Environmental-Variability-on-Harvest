...
\section{Permit portfolios and sectoral counterfactual}\label{appendix-portfolio}
...
(\citeproc{ref-Ley21752-2025}{Congreso Nacional de Chile 2025}), in force through \(2040\), and contrasts it with the
...
scientific-observer typology (\citeproc{ref-IFOP2016-observadores}{Vega Muñoz et al. 2016}): the mean number of distinct species
...
\subsection{Policy counterfactual under the 2025 reform}\label{appendix-counterfactual}
...
(\citeproc{ref-Ley21752-2025}{Congreso Nacional de Chile 2025}) fixes a new schedule of artisanal--industrial fractions
...
\begin{table}[H]
\centering
\caption{Posterior-propagated artisanal absolute landing under the pre-reform and 2025 fractioning regimes, $\mathrm{SSP}5$-$8.5$ end-of-century, Centro-Sur (Valparaíso--Los Lagos), $2024$ Cuotas Globales.\label{tab:counterfactual}}
\begin{threeparttable}
...
\midrule
Anchoveta (Valparaíso--Los Lagos) & Pre-reform & $17.1$ $[14.0, 20.8]$ & $[2.9, 75.5]$\\
Anchoveta (Valparaíso--Los Lagos) & 2025 reform & $19.7$ $[16.2, 24.0]$ & $[3.3, 87.2]$\\
Sardine (Valparaíso--Los Lagos) & Pre-reform & $0.3$ $[0.1, 1.4]$ & $[0.1, 1.7]$\\
Sardine (Valparaíso--Los Lagos) & 2025 reform & $0.3$ $[0.1, 1.6]$ & $[0.1, 1.9]$\\
Jack mackerel (Valparaíso--Los Lagos, aggregate) & Pre-reform & $65.9$ $[65.9, 65.9]$ & $[65.9, 65.9]$\\
Jack mackerel (Valparaíso--Los Lagos, aggregate) & 2025 reform & $185.7$ $[185.7, 185.7]$ & $[185.7, 185.7]$\\
\textbf{Total Centro-Sur} & Pre-reform & $83.3$ $[80.0, 88.1]$ & $[68.8, 147.4]$\\
\textbf{Total Centro-Sur} & 2025 reform & $205.7$ $[201.9, 211.3]$ & $[189.1, 279.7]$\\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
\item \emph{Notes.} Cross-model median and IQR are taken across the six CMIP6 models of the per-model posterior median; the within-posterior 90\% CI is the median across models of the per-model posterior $[q_{05}, q_{95}]$. The construction parallels the trip comparative-statics table of the main text; see \texttt{R/run\_counterfactual\_ley21752.R} for the pipeline. Jack mackerel is held at $\Delta r^{\star}/r^{0} = 0$ by the non-identification convention of the Stock dynamics subsection of the main text; anchoveta and sardine use the by-model summaries written by the main T5 pipeline (\texttt{tables/growth\_comparative\_statics\_by\_model.csv}).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\caption{Inter-sectoral cession flow, anchoveta and sardine, Central-South small pelagic fishery, $2013$--$2024$. Source:
SERNAPESCA quota control reports.\label{tab:cesiones-anchsard}}
\begin{threeparttable}
\begin{tabular}{lrrrr}
\toprule
Year & Species & IND assigned (kt) & IND cession (kt) & Share ceded \\
\midrule
\multicolumn{5}{l}{\emph{Panel A. Per-titular LTP workbooks (2013--2017)}} \\
...
\midrule
\multicolumn{5}{l}{\emph{Panel B. Consolidated RESUMEN sheet (2018--2024)}} \\
...
Cuota Global de Captura aggregated across the Valparaíso--Los Lagos
sub-macrozones. ``IND cession'' is the net industrial-side movement of
quota out of the industrial pool: \emph{negative} values denote
industrial-to-artisanal flows, \emph{positive} values denote the reverse
direction. ``Share ceded'' is the absolute ratio
$|\text{cession}|/\text{IND assigned}$. Panel~A is reconstructed from the
$2013$--$2017$ per-titular SERNAPESCA quota-control workbooks
(\emph{Pelagicos LTP} sheet for $2014$--$2017$; \emph{Pelagicos LMC y
LTP} sheet for the $2013$ LMC-to-LTP transition); cessions are summed
across titulares and sub-periods. Panel~B is taken from the SERNAPESCA
consolidated \emph{RESUMEN} sheet published from $2018$ onwards. The
$2018$ report uses a per-titular online layout and is parsed analogously
to the workbook series. The legal regime (Ley~$20{,}657$, in force from
February~$2013$) is common to the full window. The reconstructed
Panel~A series is archived in
\texttt{cesiones\_ind\_2013\_2017\_rebuilt.csv} and the full per-year,
per-sub-macrozone CSV in
...
\centering\small
\renewcommand{\arraystretch}{0.95}
\caption{Inter-sectoral cession flow, jack mackerel, Central-South, $2013$--$2024$. Source:
SERNAPESCA quota control reports.\label{tab:cesiones-jurel}}
\begin{threeparttable}
\begin{tabular}{lrrrr}
\toprule
Year & Species & IND assigned (kt) & IND cession (kt) & Share ceded \\
\midrule
\multicolumn{5}{l}{\emph{Panel A. Per-titular LTP workbooks (2013--2017, V-IX + XIV-X)}} \\
...
\midrule
\multicolumn{5}{l}{\emph{Panel B. Consolidated RESUMEN sheet (2018--2024, XV--X)}} \\
...
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
\item \emph{Notes.} Column conventions follow Table
\ref{tab:cesiones-anchsard}: negative ``IND cession'' values denote
industrial-to-artisanal flows, positive values denote the reverse
direction. Panel~A reconstructs the industrial-side cession from the
$2013$--$2017$ per-titular SERNAPESCA workbooks, summing the
\emph{Pelagicos LTP} sub-bloques \emph{Jurel V-IX} and \emph{Jurel XIV-X}
(the two Central-South management units, summed to match the
$2018$--$2024$ aggregation). Panel~B is taken from the SERNAPESCA
consolidated \emph{RESUMEN} sheet and aggregates the Arica y
Parinacota--Los Lagos (XV--X) regional rows. Panel B therefore includes
the small Arica-Antofagasta (XV--IV) volumes absent from Panel A; this
inflates the Panel~B IND-assigned column relative to Panel~A but does
not affect the qualitative reading (cession share remains below the
range observed for anchoveta and sardine in Table
\ref{tab:cesiones-anchsard} in every year of the full $2013$--$2024$
window, and the cession direction is bidirectional across years).
The reconstructed Panel~A series is archived alongside the anchoveta
and sardine rows in \texttt{cesiones\_ind\_2013\_2017\_rebuilt.csv}.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\ref{tab:cesiones-anchsard} in every year of the full \(2013\)--\(2024\)
window, and the cession direction is bidirectional across years).
The reconstructed Panel\textasciitilde A series is archived alongside the anchoveta
and sardine rows in \texttt{cesiones\_ind\_2013\_2017\_rebuilt.csv}.
\textbackslash end\{tablenotes\}
...
\bibitem[\citeproctext]{ref-IFOP2016-observadores}
Vega Muñoz, Rodrigo, Luis Ossa Medina, Benjamín Suárez Ahumada, et al. 2016. \emph{Programa de Observadores Cient{í}ficos, 2015}. Informe Final, Convenio de Desempe\{\textbackslash\textasciitilde n\}o 2015, Prepared for Subsecretar\{\textbackslash\textquotesingle i\}a de Econom\{\textbackslash\textquotesingle i\}a y Empresas de Menor Tama\{\textbackslash\textasciitilde n\}o P-581110. Instituto de Fomento Pesquero (IFOP).
