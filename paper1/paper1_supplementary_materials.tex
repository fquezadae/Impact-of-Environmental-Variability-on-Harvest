...
\textbf{Total} & Pre-reform & $83.3$ $[80.0, 88.1]$ & $[68.8, 147.4]$\\
\textbf{Total} & 2025 reform & $205.7$ $[201.9, 211.3]$ & $[189.1, 279.7]$\\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
\item \emph{Notes.} Cross-model statistics across six CMIP6 models of the per-model posterior median; within-posterior 90\% CI is the median across models of per-model $[q_{05}, q_{95}]$. Jack mackerel is held at $\Delta r^{\star}/r^{0} = 0$ (non-identified at Centro-Sur scale).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
series, which we report in two tables separated by species pattern:
...
\(2013\)--\(2017\) and tightens to \(85\%\)--\(\sim\!100\%\) over \(2018\)--\(2024\). The cession volume scales with the TAC and,
...
\(2013\)--\(2014\) and \(2016\)--\(2022\) subperiods, while industrial-to-artisanal
...
proportionally smaller change in realised artisanal landings. The operationally meaningful change in the small pelagics is the
re-pricing of the inter-sectoral cession channel. Under the
pre-reform regime, cession contracts were private agreements between
LTP-holders and RPA-holders---partially observable for those
registered with SUBPESCA, partially informal
(\citeproc{ref-Paredes2024-cuotas}{Paredes Letelier et al. 2024})---and carried no fiscal levy per tonne
transferred. The 2025 reform introduces a \(1.3\) UTM/ton fiscal patent
on each cession contract (with a \(250\) UTM/contract floor); this
shifts the binding margin of the institutional re-allocation from
the de jure share reassignment to the fiscal cost of the operational
channel.
