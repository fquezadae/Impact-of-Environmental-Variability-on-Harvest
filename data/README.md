...
├── precios/           (not tracked) Fishmeal / process prices (Banco Central, IFOP).
...
- `data/trips/poisson_dt.rds` — vessel-year panel feeding the
  ...
  days, etc.). Generated by `R/04_models/poisson_model.R`. This is a
  vessel-year aggregate: one row per vessel × year, with no
  per-haul/operation-level records. The raw IFOP logbook stays in
  `data/logbooks/` (gitignored).
...
- the CMIP6 ensemble CSV and the three projection-table CSVs in
  `tables/` (the manuscript reads them via `here::here("tables/...")`;
  `paper/tables` is only a symlink to it)
